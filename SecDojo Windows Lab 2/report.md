...
### Using the cammand bellow we can find the SPNs that tied to the user that we already have access to :
![Alt text](Includes/2.png?raw=true "Title")

### As you can see there is an SPN with a user acoount that's a member of Domain admins; with the same cammand but with an extra arg we can reauest a TGS :
